        ...
        if (preg_match('/^[A-Za-z]{2,3}+-[0-9]{1,2}+-[A-Za-z]{2,3}+-[0-9]{4}$/', $vehicle_no)) {
            if (preg_match("/^[9]+[7-8]+\d{8}$/", $phone_no)) {
                ...
                if ($con->query($sql) === true) {
                    ...
                    header("Location: ../frontend/admin-edit-challan.php?id=$challan_id&success=Successfully Updated Challan");
                } else {
                    header("Location: ../frontend/admin-edit-challan.php?id=$challan_id&error=Error while editing challan");
                }
            } else {
                header("Location: ../frontend/admin-edit-challan.php?id=$challan_id&error=Invalid Phone Number");
                exit();
            }
        } else {
            header("Location: ../frontend/admin-edit-challan.php?id=$challan_id&error=Invalid Vehicle Number");
            exit();
        }
    } else {
        header("Location: ../frontend/admin-edit-challan.php?id=$challan_id&error=Invalid License Number");
        exit();
    }
} else {
    header("Location: ../frontend/admin-edit-challan.php?id=$challan_id&error=Invalid Full Name");
    exit();
}
