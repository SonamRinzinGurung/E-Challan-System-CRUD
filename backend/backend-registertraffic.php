...
if (isset($_POST['create'])) {
    ...
    if (preg_match("/^[a-zA-z]+$/", $first_name) && preg_match("/^[a-zA-z]+$/", $last_name)) {
				...
				if (preg_match("/^[9]+[7-8]+\d{8}$/", $phone_no)) {
					...
					header("Location: ../frontend/trafficregister.php?error=Invalid Phone Number");
					exit();
				}
		
		
	} else {
		header("Location: ../frontend/trafficregister.php?error=Invalid Full Name");
		exit();
	}
